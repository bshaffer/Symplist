...
CREATE TABLE comment (id BIGINT AUTO_INCREMENT, body LONGTEXT, approved TINYINT(1) DEFAULT '0', approved_at DATETIME, user_id BIGINT, root_id INT, lft INT, rgt INT, level SMALLINT, created_at DATETIME, updated_at DATETIME, rating BIGINT DEFAULT NULL, INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE symfony_plugin (id BIGINT AUTO_INCREMENT, title VARCHAR(255), description LONGTEXT, author_id BIGINT, category_id BIGINT, active TINYINT(1), slug VARCHAR(255), created_at DATETIME, updated_at DATETIME, INDEX author_id_idx (author_id), INDEX category_id_idx (category_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE symfony_plugin ADD FOREIGN KEY (author_id) REFERENCES plugin_author(id);
