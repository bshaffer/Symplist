CREATE TABLE community_list (id BIGINT AUTO_INCREMENT, title VARCHAR(255) NOT NULL, description VARCHAR(255), featured TINYINT(1), submitted_by INT, created_at DATETIME, updated_at DATETIME, slug VARCHAR(255), description_html LONGTEXT, INDEX submitted_by_idx (submitted_by), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE community_list_item (id BIGINT AUTO_INCREMENT, title VARCHAR(255) NOT NULL, body LONGTEXT, list_id BIGINT, score BIGINT DEFAULT 0, count BIGINT DEFAULT 0, submitted_by INT, created_at DATETIME, updated_at DATETIME, body_html LONGTEXT, INDEX list_id_idx (list_id), INDEX submitted_by_idx (submitted_by), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE community_list ADD FOREIGN KEY (submitted_by) REFERENCES sf_guard_user(id);
ALTER TABLE community_list_item ADD FOREIGN KEY (submitted_by) REFERENCES sf_guard_user(id);
ALTER TABLE community_list_item ADD FOREIGN KEY (list_id) REFERENCES community_list(id);
...
ALTER TABLE plugin_release ADD FOREIGN KEY (plugin_id) REFERENCES symfony_plugin(id);
