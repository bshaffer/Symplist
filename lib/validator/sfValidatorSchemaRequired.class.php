...
class sfValidatorSchemaRequired extends sfValidatorSchema
{
   ...
   * @see sfValidatorBase
   */
  public function __construct(array $fields = null, $options = array(), $messages = array())
  {
  ...
  protected function doClean($values)
  {
    ...
    $fields = $this->getOption('fields') ? $this->getOption('fields') : array_keys($values);
    ...
    {
      $this->throwError($valid, 'min_required', $fields);
    }
    
    if ($valid > $this->getOption('max_required', $fields))
    {
  ...
  protected function throwError($numValid, $error, $fields)
  {
    ...
    if ($this->getOption('throw_global_error')) 
    {
      ...
      $errorSchema->addError(new sfValidatorError($this, 'global_invalid', array(
          ...
          'num_required'  => $error == 'min_required' ? $this->getOption('min_required') : $this->getOption('max_required')
      )));
    }

    if ($this->getOption('throw_field_error')) 
    {
